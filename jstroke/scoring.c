...
/*
 * Copyright (C) 1999  Robert E. Wells, Anthony Wong
 *
 * $Id: scoring.c,v 1.2 2000/01/12 00:54:20 ypwong Exp $
 *
 */
...
/* rsp - user's written stroke, containing all the points
 * iStrokeCnt - the number of strokes
 */
...
Long     StrokeScorerProcess  (StrokeScorer *pScorer, Long iMaxCnt) {
	...
	/* For all strokes that have the same no. of strokes in the database... */
#ifdef ORIG_SCORING
	for (cp = pScorer->m_cpStrokeDic; *cp || *(cp+1); cp = cpNext) {

		iCnt++;
		if (iMaxCnt >= 0 && iCnt > iMaxCnt)
			break;  /* so this doesn't matter if iMaxCnt == -1 */

		cpNext = StrokeScorerEvalItem(pScorer, cp, &iScore);
        /* score for this stroke is now in iScore */
        ...
        /* Code below is for Sorting the scores??  --ypwong */
...
/*  pScorer - the 'scorer' under evaluation
 *  cpEntry - the stroke entry in database for comparison
 *  ipScore - output.. score (?)
 */
CharPtr StrokeScorerEvalItem(StrokeScorer *pScorer, CharPtr cpEntry,
							 ULong* ipScore /*OUT*/) {
	CharPtr cp = cpEntry;
	UInt    iStroke;
    /* 'converted' (conversion as seen below) data will be put into cpPath,
     * and cpPathEnd will point to the end of cpPath.
     */
...
fprintf(stderr,"Testing U+%02hx%02hx, ",0x00FF&(*cp),0x00FF&(*(cp+1)));
        cp += 2;
...
//	iScore = SqrtULong(iScore); // commented because the score
                // will easily become 0 when there are many filters.
	*ipScore = iScore;

	MemoWrite2d(" is=", iScore); /* DEBUG: overall stroke score */

fprintf(stderr, " initial score = %lu\n", *ipScore);
...
fprintf(stderr, "\tfinal score = %lu\n", *ipScore);
        if ( *cp == '\0' )      /* now we use NULL char to separate each stroke */
            cp++;
	return cp;
}
...
/* bpX, bpY - all x and y stroke points
 * iLen - how many pairs of (bpX,bpY)
 * cpPath - pointing at the beginning of this test stroke (converted data)
 * iPathLen - number of segments of this test stroke (a segment is
 *      defined as the basic unit of a stroke. A stroke may be composed of
 *      one or more 'segments'
 * iDepth - 0
 */
ULong StrokeDicScoreStroke(Byte* bpX, Byte* bpY, UInt iLen,
						   CharPtr cpPath, UInt iPathLen, UInt iDepth) {
